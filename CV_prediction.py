    ...
    for (example_index, example) in enumerate(examples):
        ...
        if is_training:
            ...
            if pos == -1:
                print(doc_tokens)
                print(answer_tokens)
                # print(compute_position(doc_tokens, answer_tokens))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    final_hidden = create_eemodel(model.get_all_encoder_layers(), input_mask)
...
def main(_):
    ...
    for fold_i in range(5):
        ...
        test_writer = FeatureWriter(
            filename=os.path.join(model_dir, "test.tf_record"),
            is_training=False)
        test_features = []
